...
def test_if_template_filled():
	...
	assert_equals(config['scraping']['url'], "www.google.com")


def test_crawler_generation():
	args = docopt(doc, "genconfig project1 www.google.com -t crawler")
	gc = genconfig.GenconfigCommand(args)
	gc.execute_command()
	with open(os.path.join(os.getcwd(), 'project1.json'), 'r') as f:
		config = json.load(f)
	next = {"follow_link": "", "data": [{"field": "", "selector": ""}]}
